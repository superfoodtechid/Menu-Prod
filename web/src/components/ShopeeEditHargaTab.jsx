      ...
      fetch(`${API_BASE_URL}/api/shopee/otp-status`, {
        ...
        .then(data => {
          if (data && data.waiting) {
            setOtpModal(prev => {
              const newErrMsg = data.error_msg || "";
              if (prev.isOpen && prev.username === data.username && prev.statusMsg === newErrMsg) return prev;
              return {
                ...prev,
                isOpen: true,
                    ...
                    {catItems.map(item => {
                      ...
                      return (
                        <tr key={item.id} className={`transition ${
                          ...
                          <td className="p-3.5 text-center align-middle">
                            {item.is_flash_sale ? (
                              <span title="Harga menu dikunci karena sedang dalam promo Flash Sale aktif" className="px-2.5 py-1 rounded-lg bg-amber-100 dark:bg-amber-950/60 text-amber-800 dark:text-amber-300 text-[11px] font-bold border border-amber-300 dark:border-amber-800 inline-block">
                                ⚡ Flash Sale (Dikunci)
                              </span>
                            ) : isViolation ? (
